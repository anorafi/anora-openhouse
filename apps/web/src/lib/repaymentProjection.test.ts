...
it("reconciles portfolio value and adds only scheduled returns without duplicating principal", () => {
  ...
  expect(repaymentProjection(facilities, 30, now).at(-1)?.value).toBe(430);
  const forecast = repaymentProjection(facilities, 90, now);
  expect(forecast[0].value).toBe(430);
  expect(forecast.at(-1)?.value).toBeCloseTo(450);
  expect(forecast.at(-1)?.principal).toBe(200);
  expect(forecast.at(-1)?.returns).toBeCloseTo(20);
  expect(repaymentProjection([], 90, now).at(-1)?.value).toBe(0);
});
...
it("updates today's balance and projected returns after a new supply or top-up", () => {
  ...
  expect(initial.at(-1)?.value).toBe(130200);
  const toppedUp = repaymentProjection([{ ...position, supplied: 150000 }], 90, now);
  expect(toppedUp[0].value).toBe(150000);
  expect(toppedUp.at(-1)?.value).toBe(162750);
  expect(repaymentProjection([{ ...position, stage: "settled" }], 90, now).at(-1)?.value).toBe(0);
});
